...
public class caddieServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!request.getParameterMap().containsKey("product"))
        ...
        try {
            rs.next();
            bd.executeUpdate("INSERT INTO reservations (id_client, numero_serie, date_reservation) values ('"+client.shortValue()+"', '"+rs.getString("numero_serie")+"', sysdate)");
                bd.executeUpdate("UPDATE appareils set etat_situation = (select id_situation from etats_situation where Description = 'Réservé') where numero_serie = '"+rs.getString("numero_serie")+"'");
                session.setAttribute("flash", "Article est ajouté.");           
                bd.commit();
